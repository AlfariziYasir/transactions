...
func (s *stockService) Reserve(ctx context.Context, req *model.ReserveStock) error {
	now := time.Now()
	...
	for _, item := range req.Items {
		...
		if err != nil {
			s.log.Error("failed to reserve stock", zap.String("product_id", item.ProductID), zap.Error(err))
			return err
		}
